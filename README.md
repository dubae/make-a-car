...
**Phase 2 — 조립 (2분)**: 차고로 순간이동되고 셔터가 닫힙니다. **모터 4개는 기본 지급**! 파츠를 들고 다른 파츠에 가까이 대면 초록색 부착 후보가 표시되고, 우클릭으로 붙입니다(`R`로 분해, `Z`/`X`/`C`로 90° 회전). 몸체에 모터를 달고 **모터의 빨간 축에 바퀴 역할 파츠**를 다세요.
