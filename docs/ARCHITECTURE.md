...
- **모터** (`src/world/Motor.ts`): 개별 모터(하우징 + 빨간 축, 로컬 +X)와 **차축 모터**(얇은 축 양 끝에 바퀴 장착). Phase 3에서 모터에 붙은 파츠가 레이캐스트 바퀴가 된다.
- **운반 모드(도미넌스)**: 드는 순간 결합체 전체의 도미넌스 그룹을 최하(-127)로 낮추고 CCD를 켠다 — 충돌이 유지되어 **바닥/벽/다른 파츠를 뚫지 못하지만**, 상대는 무한 질량처럼 동작해 **기존 구조물이 절대 밀려나지 않는다** (`Grabber.setClusterCarried`). 결합체 90° 회전은 피벗 기준 보간 애니메이션(0.18초)으로 부드럽게 처리.
- **저장**: 페이즈 종료 시 파츠(형태/크기/색/자세)와 본드 그래프를 localStorage(`makeacar.car`)에 직렬화 — Phase 3가 이것으로 차를 재구성한다.

## Phase 3 레이싱 (구현됨)

`src/game/RaceManager.ts` — Rapier 내장 **`DynamicRayCastVehicleController`**(Bullet raycast vehicle 포팅) 기반:
- **차 재구성**: 조립 그래프에서 모터를 포함한 최대 클러스터를 차로 선정. 루트(차체)는 본드 최다 파츠. 각 모터의 본드 중 루트가 속한 성분은 차체, 나머지 가지는 바퀴로 분류.
- **섀시 병합**: 바퀴를 제외한 모든 파츠의 콜라이더(밀도/마찰 유지)를 **하나의 섀시 강체**로 병합 — 질량 분포가 그대로 유지되고, 조인트 사슬이 없어 발작 회전이 원천적으로 없다. 원본 파츠 강체는 비활성화하고, 매 프레임 섀시/바퀴 자세를 되돌려 써서 기존 `syncMesh` 경로로 렌더링한다. ⚠️ 텔레포트 직후 콜라이더 월드 자세는 스텝 전 상태이므로 병합 전에 `propagateModifiedBodyPositionsToColliders()` 필수.
- **바퀴 = 서스펜션 레이캐스트**: 바퀴 서브트리의 지오메트리 AABB에서 반지름을 재고, 조립 위치보다 아래로 더 뻗을 droop 여유를 준 스프링/댐퍼 서스펜션으로 등록. 접지한 바퀴만 엔진 힘을 받는다(공중 바퀴 추진 금지). `updateVehicle`은 고정 물리 스텝과 동기(`Game.tick`의 스텝 루프 안).
- **구동/조향**: 전 모터 동일 출력 — 총 힘 = 모터 수 × 상수, 속도에 비례해 감쇠(자연스러운 최고 속도). 차 무게·바퀴 반지름이 성능을 가른다. 조향은 전방(-Z 쪽) 바퀴 스티어링(축이 하나면 전체).
- **카메라**: 차 회전과 독립인 마우스 궤도 카메라. 시선 목표→카메라 방향으로 **정적 지형 레이캐스트**를 걸어 벽/가구 뒤로 나가지 않게 거리를 클램프한다(보간 후 최종 위치도 재클램프).
- **코스**: 방을 한 바퀴 도는 토러스 게이트 4개, 순서 통과 판정(수평 거리), 완주 기록.

## 확장 계획

- **멀티플레이**: 시드 공유 + 상태 스냅샷 동기화(WebSocket). 물리는 서버 권위 또는 락스텝 검토.
- 전용 트랙 맵, AI 고스트, 사운드/파티클.
