...
- **차 재구성**: 조립 그래프에서 모터를 포함한 최대 클러스터를 차로 선정. 루트(차체)는 본드 최다 파츠. 각 모터의 본드 중 루트가 속한 성분은 차체(고정 유지), 나머지는 바퀴로 분류.
- **바퀴 변환(허브 방식)**: rapier JS의 RevoluteJoint는 회전축을 양쪽 로컬 프레임에서 같은 좌표로 해석하므로, 모터와 **같은 회전의 "허브" 강체**를 축 지점에 만들고 (모터↔허브 회전 조인트, 축 (1,0,0) 공유) 허브↔바퀴는 상대 자세 유지 고정 조인트로 연결한다. 바퀴 하위 트리 전체가 함께 회전한다.
- **구동**: 전 모터 동일 `configureMotorVelocity` — 바퀴 회전 방향은 축 방향에서 자동 유도(a×up 부호). 조향은 차체 요 토크(질량 비례). 성능 차이는 물리엔진이 질량/바퀴 형상에서 만든다.
- **카메라**: 루트 회전 델타로 전방을 추정하는 3인칭 추적(지수 감쇠 lerp).
