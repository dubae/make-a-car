...
| 영역 | 선택 | 이유 |
| --- | --- | --- |
| 렌더링 | **Three.js** | 웹 표준 WebGL 렌더러. `MeshToonMaterial` + 그라디언트 램프로 토이스토리풍 셀 셰이딩 구현 |
...
외부 에셋(모델/텍스처) 없이 **프리미티브 지오메트리 + 코드 생성 텍스처**만 사용한다. 로딩이 빠르고 라이선스 문제가 없으며, 장난감 특유의 단순한 도형 미학과도 맞는다.
...
├── world/
│   ├── materials.ts         # 토온 그라디언트 램프, 장난감 색 팔레트
│   ├── ToyRoomMap.ts        # 정적 맵: 바닥/벽/테이블/책더미/연필/크레용/블록아치
...
- 동적 강체 ~60개 + 정적 콜라이더 ~30개 — 데스크톱/모바일 브라우저에서 충분히 가벼움.
- 그림자 맵 2048², `setPixelRatio ≤ 2` 제한.
