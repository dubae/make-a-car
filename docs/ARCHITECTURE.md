...
- **자유 접착(ToTK 스타일)**: 스냅 포인트 없이, 들고 있는 파츠와 다른 파츠의 표면 간격이 일정 이하면 부착 후보로 하이라이트. 클릭 시 **셰이프캐스트로 표면까지 스냅**한 뒤 현재 상대 자세를 유지하는 `JointData.fixed` 조인트로 용접한다 (`src/game/Assembly.ts`).
- **분해**: 파츠를 조준하고 `R` — 그 파츠에 연결된 본드를 모두 제거.
- **모터** (`src/world/Motor.ts`): 하우징 + 빨간 축(로컬 +X). Phase 3에서 모터에 붙은 파츠가 축 기준 RevoluteJoint로 회전한다.
- **저장**: 페이즈 종료 시 파츠(형태/크기/색/자세)와 본드 그래프를 localStorage(`makeacar.car`)에 직렬화 — Phase 3가 이것으로 차를 재구성한다.

## Phase 3 확장 계획

- **Phase 3 (레이싱)**: 저장된 그래프에서 모터-바퀴 본드를 `RevoluteJoint` + `configureMotorVelocity`(전 차량 동일 토크)로 변환. 차량 성능 차이는 물리엔진이 파츠 질량/형상에서 자동으로 만들어낸다. 카메라는 3인칭 추적으로 전환.
